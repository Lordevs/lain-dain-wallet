...
import { computeLocalCategoryBreakdown, computeLocalMonthlyTrend, computeLocalSummary } from '@/lib/my-expenses-local'
    ...
    queryFn: async () => {
      ...
      if (!onlineManager.isOnline() && ownerId) {
        ...
        if (settings && profile?.default_currency) {
          ...
          return {
            ...computeLocalSummary(input),
            by_category: computeLocalCategoryBreakdown(input),
            monthly: computeLocalMonthlyTrend(input),
          }
        }
      }
