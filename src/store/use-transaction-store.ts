...
interface TransactionState {
  ...
  getContactTransactions: (contactId: string, contactName: string) => TransactionRecord[]
...
const INITIAL_TRANSACTIONS: Record<string, TransactionRecord[]> = {
  '1': [
    {
      id: 't1',
...
export const useTransactionStore = create<TransactionState>((set, get) => ({
  transactionsByContact: INITIAL_TRANSACTIONS,

  getContactTransactions: (contactId, contactName) => {
    const { transactionsByContact } = get()
    if (transactionsByContact[contactId]) {
      return transactionsByContact[contactId]
    }
    ...
    // Save generated fallback to avoid duplicate generations
    set((state) => ({
